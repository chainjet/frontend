...
export function IntegrationFilters (props: Props) {
  ...
  return (
    <>
      <Menu selectedKeys={[categorySelected ? categorySelected.id : 'all']}
            onSelect={info => handleItemSelect(info.key.toString())}>
        <Menu.Item key="all">
          { useCategoryLink ? <Link href={useCategoryLink(null)}><a>All</a></Link> : 'All' }
        </Menu.Item>
        {
          data.integrationCategories.map(category => (
            <Menu.Item key={category.id}>
              { useCategoryLink ? <Link href={useCategoryLink(category)}><a>{category.name}</a></Link> : category.name }
            </Menu.Item>
          ))
        }
      </Menu>
